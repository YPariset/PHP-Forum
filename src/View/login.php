    ...
    <div class="flex items-center justify-center h-full">
        <div class="h-full w-full md:flex md:justify-center md:items-center md:w-form md:h-max bg-zinc-700 p-6 md:rounded-lg md:shadow-lg">
            <div class="grid gap-4 w-full mt-10 md:mt-0">
                ...
                <form id="login" class="grid gap-4" action="index.php?mod=user&action=login" method="post">
                ...
                <span class="flex items-center text-md text-thin text-zinc-600">
                    Need an account ?
                    <a class="ml-2 text-sky-600 transition-all duration-150 ease-in-out hover:text-sky-500 hover:transition-all hover:duration-150 hover:ease-in-out " href="index.php?mod=user&action=signup">Sign up</a>
                </span>
<?php
    if(isset($emailError)) {
?>
