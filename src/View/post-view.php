...
?>
            <div class="post__wrapper relative px-4 py-2 <?= isset($responseTo) ? ($responseTo["user"]["_id"]['$oid'] == $_SESSION["oid"] ? "hover:bg-yellow-700" : "hover:bg-zinc-800") : "hover:bg-zinc-800" ?>">
                ...
                <a class="response-btn absolute right-0 top-0 p-2 mr-4 bg-zinc-800 border border-zinc-700 hover:bg-zinc-900" href="/licence19/Projects/PHP-Forum/src/index.php?mod=post&action=<?= $_GET["action"] == "user" ? "user&oid=".$_GET["oid"] : "home" ?>&response=<?= $post["post"]["_id"]['$oid'] ?>">
